...
public class MyFilter implements Filter {
	...
	public void init(FilterConfig filterConfig) throws ServletException {
		...
		while(paramNames.hasMoreElements()){
			...
			System.out.println("过滤器配置的参数 name = "+name+" , value = "+filterConfig.getInitParameter(name));
		}
	 ...
	 * 如果调用了FilterChain的doFilter()方法，就会运行下一个过滤器，如果没有下一个过滤器了，就调用请求目标Servlet的service()方法，
	 * 如果因为某个情况(如用户没有通过验证)而没有调用FilterChain的doFilter()方法，则请求就不会继续交给接下来的过滤器或目标Servlet,
	 * 这时候就是所谓的拦截请求(从Servlet的观点来看，根本不知道浏览器有发出请求)。
	 * 在陆续调用完FIlter实例的doFilter()仍至Servlet的service()之后，流程会以堆栈顺序返回，所以在FilterChain的doFilter()运行完毕后，
	 * 就可以针对service()方法做后续处理。
	 * 
	 * 只需要知道FilterChain运行后会以堆栈顺序返回即可。在实现Filter接口时，不用理会这个Filter前后是否有其他Filter，应该将之作为一个独立的元件设计。
	 * Servlet/JSP提供的过滤器机制，其实是Java EE设计模式中Interceptor Filter模式的实现。如果希望可以弹性地抽换某功能地前置与后置处理元件
	 * (例如Servlet/JSP 中Servlet的service()方法的前置与后置处理)，就可以应用Interceptor Filter模式。
	 */
	@Override
